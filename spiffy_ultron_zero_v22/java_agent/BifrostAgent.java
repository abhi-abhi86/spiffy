...
public class BifrostAgent {
    ...
    public static String generateToken(String ip, int port) {
        try {
            ...
            if (octets.length != 4) return "INVALID_IP";
    ...
    public boolean connect() {
        try {
            ...
            // In production: perform ECDH key exchange here
            // For now, use a simple shared key
    ...
    public void sendMessage(String message) {
        try {
            byte[] encrypted = encryptAES(message.getBytes());
    ...
    /**
     * Generate AES key (simplified - use ECDH in production)
     */
